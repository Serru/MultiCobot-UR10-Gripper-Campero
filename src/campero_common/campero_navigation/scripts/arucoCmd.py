...
def navigation():
	...
	# Fase 1: El robot gira hasta estar centrado a la marca y avanza hacia ella

	#Orientacion
	if (arucoNav.fase1==0):
		if (arucoNav.mark_y<-0.05):
			arucoNav.vel.linear.x=0.0
			arucoNav.vel.angular.z=-0.15  #Simulacion: -0.3
		elif (arucoNav.mark_y>0.05):
			arucoNav.vel.linear.x=0.0
			arucoNav.vel.angular.z=0.15  #Simulacion: 0.3
		#Acercamiento
		elif (arucoNav.mark_x>1.3):
			arucoNav.vel.linear.x=0.15  #Simulacion: 0.2
	...
	# Fase 2: El robot hace un movimiento circular hasta estar perpendicular a la marca
	if (arucoNav.fase1==1 and arucoNav.fase2==0):
		if (arucoNav.mark_theta_median<-2):
			arucoNav.vel.angular.z = -0.2 #Simulacion: 0.5
			arucoNav.vel.linear.x = 0.0
			arucoNav.vel.linear.y = -1.3*arucoNav.vel.angular.z*0.5275
		elif (arucoNav.mark_theta_median>2):
			arucoNav.vel.angular.z = 0.2 #Simulacion: 0.5
			arucoNav.vel.linear.x = 0.0
			arucoNav.vel.linear.y = -1.3*arucoNav.vel.angular.z*0.5275
		elif (arucoNav.mark_theta_median>-2 and arucoNav.mark_theta_median<2):
			arucoNav.vel.angular.z = 0.0
			arucoNav.vel.linear.x = 0.0
	...
	# Fase 3: El robot se acerca a la marca hasta estar justo enfrente de ella
	if (arucoNav.fase1==1 and arucoNav.fase2==1):
		if (arucoNav.mark_x>1.0):				#Si esta lejos avanza en x
			...
			if (arucoNav.mark_y>0.05):			#Si no esta centrado se desplaza lateralmente
				...
				arucoNav.vel.linear.y=-0.1
		elif (arucoNav.mark_x<1.0):				#Si esta cerca se detiene en x
			arucoNav.vel.linear.x=0.0
			arucoNav.vel.linear.y=0.0
			if (arucoNav.mark_y>0.05):			#Si no esta centrado se desplaza lateralmente
				arucoNav.vel.linear.y=0.1
	...
	if (arucoNav.fase1 and arucoNav.fase2 and arucoNav.fase3):
		arucoNav.vel.angular.z = 0.0
		arucoNav.vel.linear.x = 0.0
		arucoNav.vel.linear.y = 0.0
		arucoNav.fin=1
