...
def filter_sols(q_sols):
    ...
    shoulder_pan_lower_limit = -2*pi 
    shoulder_pan_upper_limit = 2*pi
...
def select(q_sols, q_d, w=[1]*6):
    ...
    for q in q_sols:
        if filter_sols(q):
            #print("solucion filtrada:")
            #print q
            error.append(sum([w[i] * (q[i] - q_d[i]) ** 2 for i in range(6)]))
        else:
            error.append(99999)
    #print("list de errors de soluciones filtradas")
    #print(error)
    print("articulaciones de referencia")
    print(q_d)
    #print("error, indice:")
    #print(error.index(min(error)))
    print("solucion escogida")
    print(q_sols[error.index(min(error))])
    print("=========================")
